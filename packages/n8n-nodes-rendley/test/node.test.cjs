...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    // ---------- Agent host (mcp) ----------
    if (p === '/api/v1/agent' && req.method === 'POST') return ok(res, { job_id: body.prompt.includes('FAIL') ? 'aj-fail' : body.prompt.includes('ASK') ? 'aj-ask' : 'aj1', project_id: body.project_id || 'pAuto', thread_id: 't1', status: 'pending', created_at: 1, updated_at: 1 });
    ...
    if (p.startsWith('/api/v1/agent/jobs/')) {
      ...
      if (state.agentPolls[id] < 2) return ok(res, { job_id: id, project_id: 'pAuto', status: 'running', last_message: 'working' });
      return ok(res, { job_id: id, project_id: 'pAuto', thread_id: 't1', status: 'completed', last_message: 'done', commands_applied: 3, commands_failed: 0 });
    }
    if (p === '/api/v1/projects/p1/uploads/import' && req.method === 'POST') return ok(res, { storage_url: 'https://cdn/imported?sig', file_hash: 'imp-hash', status: 'complete', mime_type: 'audio/mpeg', role: 'pending', original_file_name: body.file_name, media_id: 'm-imp' });
...
(async () => {
  ...
  assert.strictEqual(out[0][0].json.download_url, 'https://cdn/out.bin?sig', 'uses output.url');
  ...
  assert.strictEqual(out[0][0].json.download_url, 'https://cdn/by-hash?sig');
  ...
  assert.strictEqual(out[0][0].json.video_url, 'https://cdn/x.mp4?sig2', 'prefers output.url');
  ...
  // 13. agent promptToVideo, wait + render after (running -> completed, real 5s sleep once)
  state.jobPolls['job-export'] = 1;
  const t0 = Date.now();
  out = await run({ ...base, resource: 'agent', operation: 'promptToVideo', prompt: 'make a promo', projectId: { mode: 'id', value: '' }, files: { file: [{ url: 'https://x/a.mp4', name: 'A' }] }, additionalOptions: { threadId: '' }, waitForCompletion: true, renderAfter: true, renderSettings: {} });
  const start = last((e) => e.path === '/api/v1/agent');
  assert.deepStrictEqual(start.body, { prompt: 'make a promo', files: [{ url: 'https://x/a.mp4', name: 'A' }] });
  assert.ok(Date.now() - t0 >= 4900, 'slept between polls');
  assert.strictEqual(out[0][0].json.project_id, 'pAuto');
  assert.strictEqual(out[0][0].json.last_message, 'done');
  assert.strictEqual(out[0][0].json.video_url, 'https://cdn/x.mp4?sig2');
  assert.strictEqual(out[0][0].json.export_job_id, 'job-export');
  console.log('OK agent:promptToVideo (+renderAfter, files alias, poll sleep)');

  // 14. agent failure message
  await assert.rejects(run({ ...base, resource: 'agent', operation: 'promptToVideo', prompt: 'FAIL', projectId: { mode: 'id', value: 'p1' }, files: {}, additionalOptions: {}, waitForCompletion: true, renderAfter: false }), /agent gave up/);
  await assert.rejects(run({ ...base, resource: 'agent', operation: 'promptToVideo', prompt: 'ASK', projectId: { mode: 'id', value: 'p1' }, files: {}, additionalOptions: {}, waitForCompletion: true, renderAfter: false }), /paused job "aj-ask" to ask a question: Which clip\?/);
  ...
  assert.strictEqual(out[0][0].json.media_id, 'm-imp'); assert.strictEqual(out[0][0].json.download_url, 'https://cdn/imported?sig');
  ...
  assert.strictEqual(out[0][0].json.download_url, 'https://cdn/list9?sig');
  ...
  assert.strictEqual(out[0].length, 2); assert.strictEqual(out[0][1].json.download_url, 'https://cdn/list9?sig');
