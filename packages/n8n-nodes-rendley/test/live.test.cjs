...
(async () => {
  ...
  await step('media:list', async () => { const out = await run({ resource: 'media', operation: 'list', projectId: { mode: 'id', value: projectId } }); assert.ok(out[0].length >= 2); assert.ok(out[0].every((i) => i.json.download_url)); return `${out[0].length} uploads, statuses: ${out[0].map((i) => i.json.status).join(',')}`; });
  await step('media:getDownloadUrl by hash + fetch', async () => { const out = await run({ resource: 'media', operation: 'getDownloadUrl', projectId: { mode: 'id', value: projectId }, mediaId: '', fileHash: pngUpload.file_hash }); const url = out[0][0].json.download_url; const r = await fetch(url); assert.strictEqual(r.status, 200); const b = Buffer.from(await r.arrayBuffer()); assert.strictEqual(b.length, PNG.length); return `via=${out[0][0].json.resolved_via}, downloaded ${b.length} bytes`; });
  await step('media:getDownloadUrl by media_id', async () => { const out = await run({ resource: 'media', operation: 'getDownloadUrl', projectId: { mode: 'id', value: projectId }, mediaId: pngUpload.media_id, fileHash: '' }); assert.ok(out[0][0].json.download_url); return `via=${out[0][0].json.resolved_via}`; });
  ...
  // paid, cheap
  await step('audio:generateSoundEffect 2s WAIT', async () => { const t = Date.now(); const out = await run({ ...base, resource: 'audio', operation: 'generateSoundEffect', projectId: { mode: 'id', value: projectId }, modelId: '', aiPrompt: 'a single soft rain drop', aiOptions: { duration_seconds: 2 }, extraParams: '', estimateCostOnly: false, additionalOptions: {}, waitForCompletion: true }); const j = out[0][0].json; assert.strictEqual(j.status, 'completed'); assert.ok(j.download_url, 'download_url'); const r = await fetch(j.download_url); assert.strictEqual(r.status, 200); return `${Math.round((Date.now() - t) / 1000)}s, job=${j.job_id}, expires=${j.url_expires_at}, via=${j.media && j.media.resolved_via ? j.media.resolved_via : 'job.output'}, fetched ${r.headers.get('content-type')}`; });
  await step('video:transcribe mp3 hash WAIT', async () => { const t = Date.now(); const out = await run({ ...base, resource: 'video', operation: 'transcribe', projectId: { mode: 'id', value: projectId }, modelId: '', mediaFile: mp3Upload.file_hash, aiOptions: {}, extraParams: '', estimateCostOnly: false, additionalOptions: {}, waitForCompletion: true }); const j = out[0][0].json; assert.strictEqual(j.status, 'completed'); let peek = ''; if (j.download_url) { const r = await fetch(j.download_url); peek = ` fetched ${r.status} ${r.headers.get('content-type')}`; } return `${Math.round((Date.now() - t) / 1000)}s, result keys: ${Object.keys(j.result || {}).join(',')}, download_url=${j.download_url ? 'yes' : 'NO'}${peek}`; });

  // agent, same host
  let agentJob;
  await step('agent:promptToVideo WAIT (trivial prompt)', async () => { const t = Date.now(); const out = await run({ ...base, resource: 'agent', operation: 'promptToVideo', prompt: 'Reply with the single word OK and make no changes.', projectId: { mode: 'id', value: projectId }, files: {}, additionalOptions: {}, waitForCompletion: true, renderAfter: false }); agentJob = out[0][0].json; assert.ok(agentJob.job_id); assert.strictEqual(agentJob.status, 'completed'); return `${Math.round((Date.now() - t) / 1000)}s ${JSON.stringify({ ...agentJob, last_message: (agentJob.last_message || '').slice(0, 60) })}`; });
  ...
  await step('agent:promptToVideo (no wait) then cancelJob', async () => { const out = await run({ ...base, resource: 'agent', operation: 'promptToVideo', prompt: 'Reply with the single word OK and make no changes.', projectId: { mode: 'id', value: projectId }, files: {}, additionalOptions: {}, waitForCompletion: false, renderAfter: false }); const j = out[0][0].json; assert.ok(j.job_id); const c = await run({ resource: 'agent', operation: 'cancelJob', agentJobId: j.job_id }); return `started status=${j.status}; cancel -> status=${c[0][0].json.status} reason=${c[0][0].json.reason}`; });
