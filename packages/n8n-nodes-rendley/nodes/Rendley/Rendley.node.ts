...
async function pollAgentJob(
	...
	while (true) {
		...
		if (Date.now() - start > timeoutMs) {
			throw new NodeOperationError(
				ctx.getNode(),
				`The Rendley agent job "${jobId}" did not finish within ${
					timeoutMs / 1000
				} seconds (last status: ${status ?? 'unknown'}).`,
			);
		}
...
async function pollApiJob(
	...
	while (true) {
		...
		if (Date.now() - start > timeoutMs) {
			throw new NodeOperationError(
				ctx.getNode(),
				`The Rendley job "${jobId}" did not finish within ${
					timeoutMs / 1000
				} seconds (last status: ${status ?? 'unknown'}).`,
			);
		}
...
export class Rendley implements INodeType {
	description: INodeTypeDescription = {
		...
		description:
			'Generate and edit videos with Rendley: prompt-to-video, AI media, projects, and MP4 export',
		defaults: {
			name: 'Rendley',
		},
		usableAsTool: true,
		...
		properties: [
			...
			projectLocator(
				{ show: { resource: ['agent'], operation: ['promptToVideo'] } },
				{
					defaultMode: 'id',
					description:
			...
			projectLocator(
				...
				{ required: true, description: 'Project the generated media is stored in' },
			),
			{
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if ((resource === 'agent' && operation === 'promptToVideo') || resource === 'edit') {
					...
					const fileEntries = (filesParam.file as IDataObject[]) || [];
					...
					let projectIdResult = (started.project_id as string) || projectId;
					...
					const body: IDataObject = { project_id: projectId, params };
					if (modelId) {
