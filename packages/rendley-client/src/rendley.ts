...
const HINTS: Record<number, { code: string; message: string }> = {
  ...
  402: {
    ...
    message:
      "This operation needs an active Rendley subscription and available credits. Upgrade or top up at app.rendley.com and try again.",
  },
...
export class RendleyClient {
  ...
  async runAiAction(
    action: string,
    input: { projectId: string; modelId?: string; params: Record<string, unknown> },
...
function aiBody(input: { projectId: string; modelId?: string; params: Record<string, unknown> }) {
  if (!input.projectId) {
    throw new RendleyError("A project ID is required for AI actions.", { code: "INVALID_INPUT" });
  }
  return {
...
/** Flat, snake_case summary of a job for automation outputs. */
