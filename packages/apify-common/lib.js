...
const HAND_OFF_NOTE =
  "The job is still running on Rendley; this run waited as long as its Apify timeout allowed. Run this Actor again with that ID in job_id to pick up the result. A longer run timeout lets one run wait longer.";

/**
 * Waits for a job for the rest of the run's time, until two minutes before
 * Apify stops the run (its timeout, one hour by default and settable per
 * run). When the budget ends first, a `running` item with `fields` and the
 * hand-off note is pushed and null is returned, so the caller just stops.
 * `wait` receives the poll options to pass to the client.
 */
export async function waitOrHandOff(label, wait, fields) {
  const timeoutAt = Actor.getEnv().timeoutAt;
  const timeoutMs = timeoutAt ? Math.max(new Date(timeoutAt).getTime() - Date.now() - WAIT_MARGIN_MS, 1_000) : WAIT_FALLBACK_MS;
  ...
  const onPoll = (status) => log.info(`[${label}] status: ${status} (${Math.round((Date.now() - startedAt) / 1000)} s)`);
  try {
    return await wait({ timeoutMs, pollIntervalMs: POLL_INTERVAL_MS, onPoll });
  } catch (err) {
    if (!hasCode(err, "WAIT_TIMEOUT")) throw err;
    log.warning("This run's time budget ended before the job finished. The job continues on Rendley; pass its ID as job_id to a later run.");
    await Actor.pushData({ status: "running", ...fields, note: HAND_OFF_NOTE });
    return null;
  }
}

/** The value of `promise`, or null when the API answers 404. */
