...
await step("connection rejects a wrong key", async () => {
  ...
  await assert.rejects(bad.run(load("connections/rendley/connection.imljson"), { apiKey: "not-a-key" }), /\[InvalidAccessTokenError\]/);
});
...
await step("getProject", async () => {
  const out = await rt.run(module_("getProject"), { projectId });
  checkInterface("getProject", out);
  assert.equal(out.id, projectId);
  assert.equal(out.name, "make-live-test");
...
await step("cost bodies for every file-input module are accepted", async () => {
  ...
  const cases = [
    ...
    ["upscaleVideo", { projectId, source: "https://download.samplelib.com/mp4/sample-5s.mp4" }],
    ["changeVoice", { projectId, source: mediaId, voiceId: (await rt.run(rpc("listVoices"), {}))[0].value }],
...
await step("generateSoundEffect then getJob (Rendley API) until complete", async () => {
  ...
  assert.ok(job.url && job.media_id);
...
await step("aiVideoAgent (trivial) then getJob (Rendley Agent)", async () => {
  const started = await rt.run(module_("aiVideoAgent"), { projectId, prompt: "Reply with the single word OK and make no changes." });
  checkInterface("aiVideoAgent", started);
