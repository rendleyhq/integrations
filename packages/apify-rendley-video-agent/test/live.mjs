 ...
 * a second edit. The first run is given a short Apify timeout to show the
 * hand-off, and job_id picks the job up.
 */
...
await step("a run that reaches its Apify timeout hands the agent job off as running", async () => {
  // 130 s until the timeout leaves a 10 s wait budget; the agent needs longer than that.
  const item = runActor(
    {
...
await step(`job_id picks the agent job up and waits (export ${skipRender ? "off" : "on"})`, async () => {
...
if (!skipRender) {
  await step("picking the agent job up again reuses the remembered export instead of starting another", async () => {
  ...
  await step("job_id picks the export job up by its own ID", async () => {
    assert.ok(exportJobId, "needs the previous step");
    const item = runActor({ job_id: exportJobId });
    assert.equal(item.status, "completed", item.error);
...
await step("edit the same project again in one run with export off", async () => {
  assert.ok(createdProjectId, "needs the previous step");
  const item = runActor({ prompt: "Change the text color to yellow.", project_id: createdProjectId, render: false });
  assert.equal(item.status, "completed", item.error);
  assert.equal(item.project_id, createdProjectId);
