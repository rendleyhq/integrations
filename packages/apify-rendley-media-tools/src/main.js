 ...
 * `project_id` is set. The work happens on Rendley, not in this container;
 * the run polls until it is done and returns the result. Should the run reach
 * its own Apify timeout first, it reports the job as running with its ID, and
 * a later run with `job_id` picks the result up.
 *
 * Every run pushes one item to the default dataset, and the file is streamed
 * into the run's key-value store as OUTPUT_MEDIA (transcripts as
 * OUTPUT_TRANSCRIPT), because Rendley's download URLs expire after a few hours.
 */
...
try {
  ...
  await Actor.pushData({ status: "failed", ...(action ? { action } : {}), error: message });
  await Actor.fail(truncate(message, 480));
}

// -----------------------------------------------------------------------------

/** Checks the form, starts the action's job and waits for it. */
async function start(client) {
  ...
  const item = { action, model_id: model?.id ?? requestedModel ?? null, project_id: projectId ?? null, workspace_id: workspaceId ?? null };
  ...
  if (input.estimate_only) {
    ...
    return Actor.pushData({ status: "completed", ...item, estimate_only: true, credits });
  }
  ...
  try {
    ...
    if (projectId || !hasCode(err, "SUBSCRIPTION_REQUIRED")) throw err;
    ...
    ({ jobId } = await client.runAiAction(def.endpoint, { ...target, projectId: project.id, workspaceId: undefined }));
  }
  log.info(`${action} started as job ${jobId}.`);
  item.job_id = jobId;
  const job = await waitOrHandOff(action, (opts) => client.waitForJob(jobId, opts), item);
  if (job) await finish(job, item);
}

/** Picks up a job that an earlier run had to leave at its timeout. */
async function resume(client, jobId) {
  log.info(`Checking job ${jobId}.`);
  let job = await orNull(client.getJob(jobId));
  if (!job) throw new Error(`No job with the ID ${jobId} exists on this account.`);
  ...
  if (!isTerminal(job.status)) {
    ...
    job = await waitOrHandOff(item.action ?? "job", (opts) => client.waitForJob(jobId, opts), item);
    if (!job) return;
  }
  return finish(job, item);
}

/**
 * The finished job becomes the item: the fields of `output` flattened under
 * their own names, `result_data` parsed, and the raw job as `job`. A
 * transcript is stored as JSON, anything else as the file itself.
 */
async function finish(job, item) {
  if (job.status !== "completed") {
    throw new Error(`${item.action ?? "The job"} ${job.status}: ${job.error ?? "no details"}`);
  }
