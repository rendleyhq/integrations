...
const authData = {
  apiKey: process.env.RENDLEY_API_KEY || "",
  apiBaseUrl: process.env.RENDLEY_API_BASE_URL || "https://api.rendley.com/v1",
};
...
describe("live", { skip: LIVE ? false : "set RENDLEY_LIVE=1 and RENDLEY_API_KEY" }, () => {
  ...
  after(async () => {
    ...
    await new RendleyClient(authData).deleteProject(projectId);
  });
  ...
  test("upload_media from URL, then get_media_url by media id and by hash", async () => {
    ...
    assert.ok(upload.media_id && upload.download_url);
    ...
    assert.ok(byMedia[0].download_url.startsWith("https://"));
    const byHash = await run(App.searches.get_media_url.operation.perform, { project_id: projectId, file_hash: fileHash });
    assert.ok(byHash[0].download_url.startsWith("https://"));
    const res = await fetch(byMedia[0].download_url);
  ...
  test("transcribe a library file by media id", async () => {
    ...
    assert.ok(out.transcript && typeof out.transcript === "object");
  });
  ...
  test("new_completed_job lists completed jobs with download URLs", async () => {
    ...
    assert.ok(rows[0].download_url, "first row has a fresh download URL");
  });

  test("agent: trivial edit completes, get_agent_job reads it, second job cancels", async () => {
    const out = await run(App.creates.start_prompt_to_video.operation.perform, {
      project_id: projectId,
    ...
    const second = await run(App.creates.start_prompt_to_video.operation.perform, {
      project_id: projectId,
      prompt: "Reply with the single word OK and make no changes.",
      wait_for_completion: false,
    });
    ...
    const { RendleyClient } = require("./client.helper.js");
    const canceled = await new RendleyClient(authData).cancelAgentJob(second.job_id);
  ...
  test("start_export without waiting returns a queued job", async () => {
    ...
    assert.ok(out.job_id);
    const { RendleyClient } = require("./client.helper.js");
    await new RendleyClient(authData).cancelJob(out.job_id).catch(() => {});
  });
