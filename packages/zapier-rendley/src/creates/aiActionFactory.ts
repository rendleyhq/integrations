...
export function makeAiActionCreate<I>(config: AiActionConfig<I>): Create<I & AiActionBaseInput> {
  ...
  const sample =
    config.resultKind === "transcript"
      ? jobSample({
          ...
          result: { language_code: "eng", text: "Welcome to the show." },
        })
      : jobSample({
  ...
  return {
    ...
    display: {
      ...
      description:
        config.resultKind === "transcript"
          ? `${config.description} Consumes Rendley credits.`
          : `${config.description} Consumes Rendley credits. ${URL_EXPIRY_NOTE}`,
    },
    operation: {
      inputFields: [PROJECT_FIELD, ...config.paramFields, MODEL_FIELD, PARAMS_JSON_FIELD, WAIT_FIELD],
      perform,
      sample,
      outputFields:
        config.resultKind === "transcript"
