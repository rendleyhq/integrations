...
/** A client for the stored connection. The client adds the bearer header itself. */
export function clientFor(bundle: Bundle<unknown>): RendleyClient {
  return new RendleyClient({
    apiKey: bundle.authData.apiKey,
    apiBaseUrl: bundle.authData.apiBaseUrl || undefined,
...
export const PROJECT_FIELD: InputField = {
  ...
  helpText: "The Rendley project the generated media is stored in. Create one with the Create Project action.",
};
...
export const MODEL_FIELD: InputField = {
  ...
  required: false,
  helpText:
    "Optional model ID for this action, for example `kling-v2.6` for video generation. Leave empty for Rendley's default. See the [model catalog](https://docs.rendley.com) for the list.",
};
...
export const PARAMS_JSON_FIELD: InputField = {
  ...
  required: false,
  helpText:
    'Advanced: a JSON object merged into the model parameters for fields not listed above, for example `{"aspect_ratio": "9:16"}`. Keys here override the fields above.',
};
...
/** Flatten a job for a Zap step, keeping `project_id` from the input when the job lacks it. */
