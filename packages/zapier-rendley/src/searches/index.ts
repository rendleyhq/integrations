...
export const getJob: Search<{ job: string }> = {
  ...
  display: {
    ...
    description:
      "Looks up an AI action or render job by ID and returns its status and, once complete, a fresh download URL. Pair it with a Delay step to finish jobs that outlast a single action.",
  },
  operation: {
    inputFields: [
      {
        ...
        type: "string",
        required: true,
        helpText: "The Job ID returned by an AI action or the Render Video action.",
      },
    ],
...
export const getAgentJob: Search<{ job: string }> = {
  ...
  display: {
    ...
    description: "Looks up an AI agent edit by Job ID and returns its status, message and project.",
  },
  operation: {
    inputFields: [
      {
        ...
        type: "string",
        required: true,
        helpText: "The Job ID returned by the Edit Video With AI Agent action.",
      },
    ],
    perform: async (z: ZObject, bundle) => {
      try {
...
export const getMediaUrl: Search<{ project_id: string; media?: string; file_hash?: string }> = {
  ...
  operation: {
        ...
        return [
          {
            ...
            download_url: resolved.storage_url,
            project_id,
          },
        ];
      } catch (err) {
    ...
    outputFields: [
      ...
      { key: "download_url", label: "Download URL (signed, expires)" },
...
export const estimateCost: Search<{ action: string; project_id: string; model?: string; params_json?: string }> = {
  ...
  noun: "Cost Estimate",
  display: {
    label: "Estimate Credit Cost",
    description:
      "Estimates how many Rendley credits an AI action or render would consume, without running it. Use it with a Filter step to stay within a budget.",
  },
  operation: {
    inputFields: [
      {
      ...
      },
      {
        ...
        required: true,
        dynamic: "new_project.id.name",
        helpText: "The project the action would run in. For Render Video, the project to render.",
      },
      {
        key: "model",
        label: "Model",
    ...
    perform: async (z: ZObject, bundle) => {
      ...
      try {
        ...
        return [{ id: `${action}:${credits}`, action, credits, usd: credits / 100 }];
      } catch (err) {
        throw toUserError(z, err);
      }
    },
    sample: { id: "generate-video:85", action: "generate-video", credits: 85, usd: 0.85 },
    outputFields: [
      { key: "action", label: "Action" },
      { key: "credits", label: "Credits", type: "integer" },
      { key: "usd", label: "Approximate USD", type: "number" },
    ],
  },
};
