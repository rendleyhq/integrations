...
- **Waiting never loses a job.** Jobs run on Rendley for minutes or longer. n8n polls with retries on transient errors and, when its timeout ends, fails with the job ID and a pointer to Get Job. Zapier polls for about 20 seconds and returns the job ID with Is Complete false. Make returns the job ID at once and Get Job Status reads it. The Apify Actors start the job and exit by default, pick it up later by job ID, and a waiting run waits for the rest of its own timeout before handing the job off as running.
